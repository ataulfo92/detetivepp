...
$ID            = $_POST['ID'];
$NOME          = mysqli_real_escape_string($conectar,htmlspecialchars($_POST['NOME'])); //OK
$TELEFONE      = mysqli_real_escape_string($conectar,htmlspecialchars($_POST["TELEFONE"])); //OK
...
if(isset($_POST['Alterar'])):
  ...
  $sql_atualizar = "UPDATE Comprador SET `NOME` = '$NOME',`TELEFONE` = '$TELEFONE',`EMAIL` = '$EMAIL',`STATUS`='$STATUS',`SITUACAO`='$SITUACAO' where `ID` = $ID";
  $Alterar_dados = mysqli_query($conectar,$sql_atualizar);
  ...
  if($Alterar_dados):
    ...
    echo '<div>';
    echo "Erro na alteração do comprador $NOME";
    echo '</div>';
    echo '</div>';
  endif;
...
elseif(isset($_POST['Deletar'])):
  ...
  if($Deletar_dados):
    ...
    echo '<svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Danger:"><use xlink:href="#exclamation-triangle-fill"/></svg>';
    echo '<div>';
    echo "Comprador $NOME excluído com sucesso!";
    echo '</div>';
    echo '</div>';
  else:
    echo '<svg xmlns="http://www.w3.org/2000/svg" style="display: none;">';
    ...
    echo '<svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Danger:"><use xlink:href="#exclamation-triangle-fill"/></svg>';
    echo '<div>';
    echo "Erro na exclusão do comprador $NOME";
    echo '</div>';
    echo '</div>';
  endif; 
endif;
